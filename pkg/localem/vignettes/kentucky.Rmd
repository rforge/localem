...
# number of cores for computations in parallel (only applicable to UNIX operating systems)
ncores = ifelse(.Platform$OS.type == 'unix', ceiling(0.25 * parallel::detectCores()), 1)
...
The local-EM risk estimation is done with the `riskEst()` function for the optimal bandwidth of 17.5 km found in the CV analysis. The risk is estimated at the same resolution as the fine raster. 
...
Exceedance probabilities are computed with the `excProb()` function to measure the uncertainty of the local-EM risk estimation. Under the assumption that cases are a realisation from the background population and constant risk threshold $\lambda(s) = c$, cases are bootstrapped and randomly aggregated to the counties. The local-EM risk is estimated for the bootstrapped data using the same bandwidth as the observed data. Exceedance probabilities are then computed as the percentage of the observed risk estimates at least as large as the ones of the bootstrap data at locations $s$. Large exceedance probabilities are consistent with the risk being greater than the specified threshold (i.e., $\lambda(s) > c$). 
...
for(Dthreshold in names(lemExcProb)) {
	...
	map.new(kentuckyTract, 
		mar = c(0, 0, 1, 0), 
		main = bquote(paste('Exceedance Probabilities, ', lambda(s), ' = ', .(threshold), sep = ''))
	)
