...
if(any(commandArgs()=='highRes' | basename(getwd())=='www') ) {
  ...
  nsim = 20
  nxv = 8
  path = file.path(dirname(tempdir()), paste('localEMhighRes', Sys.info()$user, sep=''))
  Sbw = unique(round(exp(seq(log(5), log(50), len=15)))) * 1000
  knitr::opts_chunk$set(dpi = 200)
  # number of cores set to total available, min 2, max 32
  if(.Platform$OS.type == 'unix')
    ncores = pmin(32, pmax(2,
            length(grep("processor", scan("/proc/cpuinfo", what='a', sep='\n', quiet=TRUE), value=TRUE)),
            na.rm=TRUE
            ))
} else {
  ...
  nsim = 3
  nxv = 4
  # on unix systems results will be saved in /tmp/localEM
...
xvAll = lemXv(
    ...
    # only 2 cores if gpu will be used
    # otherwise the gpu memory will fill up
    ncores = ifelse(requireNamespace("gpuR", quietly=TRUE),2,ncores),
    verbose = TRUE,
    path=tempdir())
```
...
matplot(xvAll$xv[,'bw']/1000, 
    ...
    xlab='km', ylab='-log p', 
    type='l', lty=1, log='x',
    ylim = quantile(unlist(xvAll$xv[,-1]), c(0, 0.8)) )
```


```{r estPlotMult, fig.cap='Simulation 2,3', fig.subcap = c('2 true','2 est','3 true','3 est')}
