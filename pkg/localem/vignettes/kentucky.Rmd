...
if(Sys.info()['user'] == 'patrick' & interactive()) {
  ...
  ncores = 8
  cellsCoarse=10
  path = '/tmp/localEM/kentuckyVignette/'
  dir.create(path, showWarnings=FALSE, recursive=TRUE)
  Sbw = c(seq(4, 16, by=2), seq(20,40,by=5)) * 1000
} else {
  ncores = 2
  cellsFine = 120
  cellsCoarse= 10
  path = tempdir()
  Sbw = c(seq(5, 40, by=2.5)) * 1000
}
...
countyCounts = as.data.frame(simplify2array(lapply(kCases$agg, table,exclude=NULL)))
countyCounts = countyCounts[!is.na(rownames(countyCounts)),]
countyCounts$id = rownames(countyCounts)
...
# simulated risk map
iCol = colourScale(
    kCases$raster$relativeIntensity[[1]], 
    breaks = 8, style = 'equal', dec = -log10(0.5))

map.new(kentuckyTract)

plot(kCases$raster$relativeIntensity[[1]], 
...
xvKentucky = lemXv(
    ...
    verbose = TRUE)
if(Sys.info()['user'] == 'patrick' & interactive()) {
  save(xvKentucky, file=file.path(path, 'xvKentucky.RDdata'))  
}
```
