...
```{r simcases, message=FALSE, cache=(basename(getwd())=='www'), cache.path='cache'}
...
```{r plotOffset, echo = FALSE, fig.cap = 'Simulated Events', fig.subcap = c('Offset', 'Relative Intensity', 'Events', 'Counts'), fig.ncol=2}
...
```{r cvPlotNew, fig.cap = 'Cross Validation Scores', fig.height=4, fig.width=6, out.width = '80%', fig.ncol=NULL}
...
```{r cvNewAll, eval=FALSE, purl=FALSE, cache=(basename(getwd())=='www'), cache.path='cache'}
library('localEM')
xvAll = lemXv(
...
```{r cvPlotAll, eval=FALSE, purl=FALSE, fig.cap='cross validation scores', fig.height=4, fig.width=6, out.width = '80%', fig.ncol=NULL}
...
```{r finalStep, eval=FALSE, purl=FALSE, cache=(basename(getwd())=='www'), cache.path='cache'}
kentuckyFinal = lemFinal(
    x=xvAll,
    counts=grep("^count[[:digit:]]", names(kentuckyCounty), value=TRUE),
    ncores = 2,
    filename = file.path('cache', 'lemFinal.grd'),
    verbose = TRUE)
```

```{r estPlotMult, eval=FALSE, purl=FALSE, fig.cap='Simulation 2,3', fig.subcap = c('2 true','2 est','3 true','3 est'), fig.ncol=2, out.width = '44%'}
...
for(Dsim in Ssim) {
  ...
  map.new(kentuckyCounty)
  plot(kentuckyFinal[[
      grep(paste('[[:alpha:]]',Dsim,'$',sep=''), 
        names(kentuckyFinal))[1] ]], 
      col = iCol$col, breaks=iCol$breaks,
      legend=FALSE,
      add=TRUE)
  plot(kMap, add = TRUE)
