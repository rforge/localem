...
if(!file.exists(theFileExcProb)) {
  ...
  for(inS in 1:nsim) {
    ...
    if(inS == 1) {
    theLemExcProbStack = theLemExcProb
    } else {
     theLemExcProbStack = raster::addLayer(theLemExcProbStack, theLemExcProb)
    }
  }
...
```{r rocPlot, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = 'Average ROC acroess all simulations', fig.height = 4, fig.width = 4, fig.ncol = length(Sthreshold), results = 'hide', fig.subcap = paste('threshold',Sthreshold)}
