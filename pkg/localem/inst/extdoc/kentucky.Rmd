...
path = file.path(dirname(tempdir()),  
  paste('localEM', Sys.info()['user'], sep = ''))
cacheDir = 'cache/'
figDir = 'figure/'
```  
...
for (i in 1:length(myLemRoc)){
  ...
  rocPlong = reshape(rocP,varying = list(2:4),idvar = c("onemspec", "model"), direction = "long", 
                   v.names = "Sensitivity",timevar = "threshold",times =as.character(gsub("^X","",names(rocP)[2:4])))
  rocPlong$sim = i
