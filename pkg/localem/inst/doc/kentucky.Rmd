...
if(FALSE) {
  ...
  smoothingRasterTemplate = brick(
      nrows=5, ncols=6, 
      xmn=0,xmx=1.2,ymn=0,ymx=1,
      nl = 2
  )
  toWrite = matrix(as.double(0.1), ncol(smoothingRasterTemplate), nlayers(smoothingRasterTemplate))
  ...
  .Call("mmapReplaceReal", 
      ...
      out, 
      PACKAGE='localEM') 
#  out[prod(dim(firstRaster))] = as.numeric(-3.2)        
 # out[c(1:4, prod(dim(firstRaster)))
  out[1:4]
  ( mmap::munmap(out)  )
...
plot(xvKentucky$xv[,1]/1000, xvKentucky$xv[,2], 
    ...
    xlab = 'km', ylab = '-log p', 
    type = 'o', xlim = c(8, 30), ylim = c(0, 20))
```
...
High-resolution plots (i.e., raster resolution of 8.5x8.5 km) of the local-EM
estimation with their optimal bandwidths are provided for all simulated
datasets. 
...
high cancer risk in the areas located east of Bowling Green and south of
...
Exceedance probabilities with risk thresholds of 1, 1.25, 1.5 and 2 are provided
for the first simulated dataset. The plots show the area east of Bowling Green
and south of Frankfurt having high lung cancer risk with the estimated risk
being higher than most bootstrap samples with the threshold of 1.5; however, the
estimated risk does not appear to exceed a threshold of 2. 

```{r excProbOne, eval=FALSE, warning = FALSE, cache=TRUE}
...
```{r plotExcProbOne, eval=FALSE, echo = FALSE, warning = FALSE, fig.cap = 'Exceedance Probabilities for Simulation 1', fig.subcap = paste('Threshold ', threshold, sep = ''), fig.height = 3, fig.width = 6, out.width = theWidth, fig.ncol = 2}
...
```{r cvPlotAll, echo = FALSE, fig.cap = 'Cross-validation Scores for All Simulations', fig.height = 4, fig.width = 6, out.width = '50%', fig.ncol = 1}
...
```{r plotRiskAll, echo = FALSE, warning = FALSE, fig.cap = 'Risk Estimation with Optimal Bandwidths', fig.subcap = paste('Simulation ', 1:4, gsub("bw", ", bw=", xvAllKentucky$bw), sep = ' '), fig.height = 3, fig.width = 6, out.width = theWidth, fig.ncol = 2}

toPlot = brick(filename(xvAllKentucky$riskEst))

rCol = colourScale(
    toPlot, 
    breaks = 8, style = 'equal', dec = -log10(0.25))
